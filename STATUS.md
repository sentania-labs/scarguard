...
- **CI/CD:** GitHub Actions workflows build and push images to GHCR. Generic CI jobs (lint, typecheck, pytest, path filtering, post-release steps) run on the lab ARC pool (`runs-on: lab`); jobs needing a real Docker daemon or multi-arch buildx stay on the 3 x86 docker runners, and Jetson builds on the 1 Orin runner. The 3 legacy generic runners are now unreferenced. Web, notifier, caddy, and detector-x86 builds parallelized across docker runners. CI lint/tests run on PRs only; main-push builds warm the GHA cache without re-running tests. Weekly cleanup hits each docker runner via a label-pinned job. Compose smoke test, GPU/CPU inference benchmarks in CI.
